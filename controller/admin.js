...
router.get('/patients/:patient_id', protected, (req, res) => {
    ...
    db.query(`SELECT fullname,contact,gender,birthdate,age,address,patient_history,guardian FROM patient_accounts WHERE id=${db.escape(patient_id)}`,
        (err, result) => {
            if (err) throw err;
...
router.get('/patients/:patient_id/:mr_id', protected, (req, res) => {
    ...
    db.query(`
        SELECT ${pa},${apt},${mr} FROM ((patient_accounts AS pa INNER JOIN medical_records AS mr ON pa.id=mr.id) INNER JOIN appointments AS apt ON apt.apt_id=mr.mr_id) WHERE mr.mr_id=${db.escape(mr_id)};
    `,
        (err, result) => {
            if (err) throw err;
...
router.get('/list/appointments', (req, res) => {
    ...
    db.query(`SELECT apt.apt_id AS apt_id,apt.schedule AS schedule,apt.status AS status,pa.fullname AS fullname,pa.contact AS contact,pa.address AS address FROM appointments AS apt INNER JOIN patient_accounts AS pa ON apt.id=pa.id WHERE ${query} ORDER BY DATE(schedule), TIME(schedule);`, (err, result) => {
        if (err) throw err;
...
router.get("/patient/medical-records/:patient_id", (req, res) => {
    ...
    db.query(`SELECT mr.ailment AS ailment,mr.date_created AS date_created,mr.mr_id AS mr_id FROM medical_records AS mr INNER JOIN patient_accounts AS pa ON mr.id=pa.id WHERE mr.id=${db.escape(patient_id)} ORDER BY DATE(mr.date_created) DESC, TIME(mr.date_created) DESC`,
        (err, result) => {
            if (err) throw err;
